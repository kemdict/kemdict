...
<h4 class="text-lg"><code>lang:{"<語言代碼>"}</code></h4>
...
<h4 class="text-lg"><code>from:{"<辭典代碼>"}</code></h4>
...
<table>
{dicts.map(({id, name}) => (<tr><td>{name}</td><td><code>{id}</code></td></tr>))}
</table>

<h4 class="text-lg"><code>title:{"<文字>"}</code></h4>
