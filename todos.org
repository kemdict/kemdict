...
* todos
** reflect db schema update

- =stroke_count= → =sc=
- =non_radical_stroke_count= → =nrsc=
- =definition= → =def=
- =definitions= → =defs=
- Collected pronunciations are in their own table and not a JSON field
...
** TODO [#A] Word page: split sections for each language like goo辞書
